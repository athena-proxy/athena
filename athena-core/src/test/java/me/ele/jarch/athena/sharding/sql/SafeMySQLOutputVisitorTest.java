...
/**
 * Created by jinghao.wang on 16/4/5.
 */
public class SafeMySQLOutputVisitorTest {
    String selectSql =
    ...
    private Predicate<String> whiteFieldsFilter = null;
    ...
    @Test public void testSetTable() throws Exception {
        ...
        SafeMySQLOutputVisitor v;
        v = new SafeMySQLOutputVisitor(mostSafeSb, whiteFieldsSb, whiteFieldsFilter);
        ...
        String expectedWhiteFields =
            "eleme_order.user_id = 100#eleme_order.restaurant_id IN (200, 300)#LIMIT 100#";
    ...
    @Test public void testSetNonInsertStmt() {
        ...
        StringBuilder whiteFieldsSb = new StringBuilder();
        SafeMySQLOutputVisitor v =
            new SafeMySQLOutputVisitor(mostSafeSb, whiteFieldsSb, whiteFieldsFilter);
    ...
    @Test(enabled = true) public void testOriginalMysqlOutPutVistor() throws Exception {
        StringBuilder debugSb = new StringBuilder();
        MySqlOutputVisitor v = new MySqlOutputVisitor(debugSb);
    ...
    @Test public void testSpecialCharExpMysqlOutPutVistor() throws Exception {
        SQLStatementParser parser = SQLParserUtils.createSQLStatementParser(
            "SELECT eleme_order.id FROM eleme_order WHERE 'bind_master'='bind_master'",
            JdbcUtils.MYSQL);
        SQLStatement statement = parser.parseStatement();
        ShardingUtil.SafeSQL safeSql = ShardingUtil
            .mostSafeSQLAndWhiteFields(Collections.singletonList(statement),
                ShardingSQL.BLACK_FIELDS_FILTER);
        Assert.assertTrue(safeSql.mostSafeSql.contains("BIND_MASTER = BIND_MASTER"));

        statement = SQLParserUtils
            .createSQLStatementParser("SELECT eleme_order.id FROM eleme_order WHERE 'aaaa'='aaaa'",
                JdbcUtils.MYSQL).parseStatement();
        safeSql = ShardingUtil.mostSafeSQLAndWhiteFields(Collections.singletonList(statement),
            ShardingSQL.BLACK_FIELDS_FILTER);
        Assert.assertTrue(safeSql.mostSafeSql.contains("? = ?"));
    }
}
